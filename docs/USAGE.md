...
- **Wakatime Today** - Show today's coding time from Wakatime (e.g. `WK 6h25m`). Reads the API key from `~/.wakatime.cfg` (honors the standard `WAKATIME_HOME` override), caches successful results for 60 seconds and failures for 30 seconds, and falls back to `WK -` when the key is missing or the request times out (2-second hard timeout); enable `hide-when-empty` to render nothing instead. Configurable format (human/digital/text/decimal), prefix, hide-when-empty, and an optional clickable OSC 8 link to the Wakatime dashboard.
