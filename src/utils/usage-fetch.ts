...
// The per-model fields (weeklySonnetUsage, weeklyOpusUsage, weeklyFableUsage)
// are declared by hand here and in UsageApiResponseSchema below because Zod
// object shapes need statically-known keys to keep field-level type inference
// for the parse* functions further down. usage-fetch.test.ts's schema-parity
// test asserts both schemas cover every WEEKLY_MODEL_USAGE_BUCKETS entry, so
// a bucket added to one but not the other fails a test instead of silently
...
// WeeklyModelUsageBucket in usage-types.ts. This is the ONLY place Fable's
// weekly quota is available: unlike Sonnet/Opus, seven_day_fable never
// existed as a real flat field.
...
// scoped to this one lookup rather than to the whole parsed object. This is
// also how Fable's (nonexistent) "seven_day_fable" key resolves safely to
// undefined: it isn't declared in the schema, so the cast just sees whatever
// passthrough value is or isn't present.
...
function parseUsageApiResponse(rawJson: string): UsageData | null {
    ...
    for (const bucket of WEEKLY_MODEL_USAGE_BUCKETS) {
        ...
        // legacy flat bucket instead of limits[]. For Fable this always
        // resolves to undefined (no such flat field ever existed) rather than
        // synthesizing a 0%.
